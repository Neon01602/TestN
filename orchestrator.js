// Forge — Agent Orchestrator (LangGraph-style state machine pipeline)
// FIX: Removed dangling `export default async function handler()` that was
//      appended outside the class — it caused a duplicate-export syntax error
//      and would break any bundler / Node ESM loader.
...
export class Orchestrator {
  ...
  async proceedToDeploy(incidentId, contextResult, patchResult, startTime) {
    ...
    recentDeploys.unshift({
      service: incident.service,
      version: `v${Math.floor(2 + Math.random() * 3)}.${Math.floor(10 + Math.random() * 20)}.${Math.floor(1 + Math.random() * 9)}`,
  ...
  async approveHumanReview(incidentId) {
    const incident = this.incidents.get(incidentId);
    if (!incident || !incident.context || !incident.patch) {
      return { error: 'Incident not found or not awaiting review' };
    }
